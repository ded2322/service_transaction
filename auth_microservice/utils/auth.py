...
def decode_jwt_user_id(token: str) -> int:
    try:
        ...
        raise HTTPException(status_code=401, content={"detail": "The access token has expired"})
    except JWTError:
        raise HTTPException(status_code=401, content={"detail": "Invalid access token format"})
